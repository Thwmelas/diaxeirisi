...
xterm -hold -e "cd $HOME/drone_ws/simulation_launch && source /opt/ros/humble/setup.bash && source $HOME/drone_ws/install/setup.bash && ros2 launch launch/swarm_launch.py" &
echo "   Περιμένω 20 δευτερόλεπτα να σταθεροποιηθεί το simulation..."
sleep 20
echo "3/4: Ξεκινάω το decision placeholder..."
xterm -hold -e "cd $HOME/drone_ws/communication_node && python3 decision_placeholder.py" &
sleep 2
echo "4/4: Ξεκινάω το perception placeholder..."
xterm -hold -e "cd $HOME/drone_ws/perception_node && source /opt/ros/humble/setup.bash && source $HOME/drone_ws/install/setup.bash && python3 perception_placeholder.py" &
echo ""
