...
Receives structured drone messages and returns JSON decisions.
"""
...
class LLMDecisionMaker:
    """Main class for interpreting drone messages.

    llm_client must be a callable: response_text = llm_client(prompt)
    If no LLM is connected, safe rule-based fallback is used.
    """
...
Mission context:
- Multiple drones cooperate in a shared environment.
- Drones send processed information, not raw images.
- Safety-critical obstacles must be prioritized.
...
Decision rules:
- If distance <= 10 meters, risk_level should usually be high.
- If object is person, fire, smoke, or vehicle, broadcast should usually be true.
- If confidence is below 0.50, recommend verification.
...
""".strip()
    ...
    def validate_decision(decision: Decision) -> Decision:
        ...
        if not decision["action"] or not decision["recommendation"]:
            raise ValueError("Action/recommendation cannot be empty")
        return decision
...
if __name__ == "__main__":
    sample = {"drone_id": "drone_1", "object": "tree", "distance": 8, "direction": "front", "confidence": 0.89}
